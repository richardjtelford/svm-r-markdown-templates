...
\moveleft.5\hoffset\centerline{ $if(email)$$if(fontawesome)$\faEnvelopeO \hspace{1 mm}$else$\emph{E-mail:}$endif$ \href{mailto:}{\tt $email$} \hspace{1 mm}$endif$ $if(phone)$$if(fontawesome)$ \faPhone \hspace{1 mm}$else$\emph{Phone:}$endif$  $phone$  \hspace{1 mm} $endif$ $if(twitter)$$if(fontawesome)$\faTwitter \hspace{1 mm}$else$\emph{Twitter:}$endif$ \href{http://twitter.com/$twitter$}{\tt @$twitter$} \hspace{1 mm} $endif$}

\moveleft.5\hoffset\centerline{$if(web)$$if(fontawesome)$\faGlobe \hspace{1 mm}$else$\emph{Web:}$endif$ \href{http://$web$}{\tt $web$}  $endif$ $if(blog)$$if(fontawesome)$\faGlobe \hspace{1 mm}$else$\emph{Blog:}$endif$ \href{http://$blog$}{\tt $blog$}  $endif$ $if(updated)$ | \emph{Updated:} \today$endif$}
\vspace{2 mm}
